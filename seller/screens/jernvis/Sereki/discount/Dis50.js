...
export default function Dis50() {
  ...
  const [originalPrice, setOriginalPrice] = useState(""); // user-entered price
  const [currency, setCurrency] = useState("$"); // $ or IQD
...
const clearForm = () => {
  ...
  const getDiscountedPrice = () => {
    ...
    return (num * 0.5).toFixed(3); // 50% discount
  };

  const handleSave = () => {
    if (name.trim() === "" || originalPrice.trim() === "") return;
    const discountedPrice = getDiscountedPrice();
   // 🔹 زیادکرا لە handleSave
const newItem = {
 ...
 const startEdit = (item) => {
  setEditItem(item);
  setName(item.name);
  const original = parseFloat(item.price.replace(currency,"")) / 0.5;
  ...
  setCustomCode(item.code || "");   // ← لێرە تەنها
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.uploadButton} onPress={() => setModalVisible(true)}>
        <Image source={require("../../../../assets/m8.png")} style={{ width: wp("20%"), height: hp("8%") }}/>
        <Text style={styles.uploadText}>Discount 50%</Text>
      </TouchableOpacity>
        ...
        renderItem={({ item }) => (
          <View style={styles.itemBox}>
            <TouchableOpacity style={{ flex: 1 }} onPress={() => startEdit(item)}>
              ...
              {item.code ? (
  <Text style={{color:"#11a1a1",fontSize:12}}>
    Code: {item.code}
  </Text>
) : null}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.label}>Price after 50% discount</Text>
              <TextInput
                value={`${getDiscountedPrice()} ${currency}`}
                editable={false}
                style={[styles.input,{backgroundColor:"#e0f7fa"}]}
              />
