...
const Reels = () => {
  ...
  const { uploadReel, updateReel, deleteReel, uploadLimit, canUploadToday } =
    useReelManagement();
  ...
  const validateForm = () => {
    ...
    if (
      !duration ||
      parseInt(duration) < MIN_DURATION ||
      parseInt(duration) > MAX_DURATION
    ) {
      Alert.alert(
        "Error",
  ...
  const createReelFormData = async () => {
    ...
    if (videoAsset) {
      ...
      const mimeType = videoInfo?.format
        ? `video/${videoInfo.format}`
        : "video/mp4";
  ...
  const handleUploadReel = async () => {
    ...
    try {
      ...
      if (selectedReel) {
        console.log("Updating reel with:", updateData);
        await updateReel({
          id: selectedReel.id,
          updates: updateData,
        }).unwrap();
  ...
  return (
    <View style={[styles.container]}>
      ...
      >
        <LinearGradient
          colors={
            canUploadToday ? ["#6366f1", "#8b5cf6"] : ["#9ca3af", "#6b7280"]
          }
